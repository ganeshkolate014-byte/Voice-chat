...
const App: React.FC = () => {
    ...
    return (
      <div className="min-h-screen neo-bg flex flex-col items-center justify-center p-6">
        <motion.div 
            ...
            className="neo-card p-10 max-w-md w-full flex flex-col gap-8 items-center text-center relative"
        >
  ...
  // --- MAIN APP CONTENT WRAPPED IN ANIMATE PRESENCE ---
  return (
      ...
      <AnimatePresence mode="wait">
        {!isInRoom ? (
          ...
          <motion.div 
            ...
            className="min-h-screen flex flex-col p-6 relative w-full"
          >
            {/* Lobby Header */}
            <div className="w-full max-w-4xl mx-auto flex items-center justify-between mb-12 mt-4">
                ...
                <div className="flex gap-4">
                    <Button 
                        ...
                        className="!h-10 !px-4 !rounded-lg border-[3px] flex items-center gap-2"
                    >
            ...
            <div className="flex-1 w-full max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                ...
                <motion.div 
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="neo-card p-8 flex flex-col gap-6 bg-white cursor-pointer"
                    onClick={() => enterRoom()}
                >
                    ...
                    <Button onClick={(e) => { e.stopPropagation(); enterRoom(); }} variant="primary" className="h-16 text-xl">
                ...
                <motion.div 
                    whileHover={{ scale: 1.02, y: -5 }}
                    className="neo-card p-8 flex flex-col gap-6 bg-[#FFFBEB]"
                >
            ...
            <div className="mt-12 text-center font-bold text-gray-400 uppercase tracking-widest text-sm pb-6">
          ...
          >
            {/* Permission Check inside Room */}
            {!myStream ? (
                <div className="flex-1 flex items-center justify-center p-6">
                    <motion.div 
                        initial={{ scale: 0.9, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                        className="neo-card max-w-lg w-full p-12 flex flex-col gap-8 items-center text-center relative"
                    >
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-black/70 text-lg font-medium">We need your microphone to transmit audio.</p>
                        </div>
                        {error && (
                ...
                <motion.div 
                    initial={{ y: 100 }}
                    animate={{ y: 0 }}
                    className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white border-[3px] border-black p-2 rounded-2xl shadow-[6px_6px_0px_0px_#000] z-50 flex items-center gap-4 scale-90 md:scale-100"
                >
                    <motion.button 
                        whileTap={{ scale: 0.9 }}
                        onClick={toggleMic}
                        className={`w-14 h-14 rounded-xl border-2 border-black flex items-center justify-center transition-colors ${isMuted ? 'bg-red-200' : 'bg-white hover:bg-gray-50'}`}
                    ...
                    <motion.button 
                        whileTap={{ scale: 0.9 }}
                        onClick={leaveRoom}
                        className="w-16 h-16 rounded-xl border-2 border-black flex items-center justify-center bg-red-500 hover:bg-red-600 transition-colors"
                    ...
                    <motion.button 
                        ...
                        className={`w-14 h-14 rounded-xl border-2 border-black flex items-center justify-center transition-colors ${isChatOpen ? 'bg-[#FDE047]' : 'bg-white hover:bg-gray-50'}`}
                        title="Chat"
                    >
                ...
                <AnimatePresence>
                    {error && (
                        <motion.div 
                            ...
                            className="fixed top-24 left-1/2 bg-[#FECACA] border-[3px] border-black text-black px-6 py-4 rounded-xl shadow-[8px_8px_0px_0px_#000] z-[100] flex items-center gap-4"
                        >
...
const ParticipantCard: React.FC<{ connection: any, onDisconnect: (id: string) => void }> = ({ connection, onDisconnect }) => {
  ...
  return (
    <motion.div 
        ...
        className={`neo-card p-6 transition-colors duration-300 relative group ${isSpeaking ? 'bg-[#F0FDFA]' : ''}`}
    >
       ...
       {isSpeaking && (
           <motion.div 
                ...
                animate={{ opacity: 1, scale: 1 }}
                className="absolute top-2 right-2 bg-[#34D399] border-2 border-black px-2 py-0.5 text-[10px] font-bold uppercase rounded shadow-[2px_2px_0px_0px_#000]"
           >
               Speaking
           </motion.div>
       )}
