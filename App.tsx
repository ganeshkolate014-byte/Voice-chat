...
import { Mic, MicOff, Users, Copy, LogOut, ShieldCheck, Share2, Sparkles, Activity, Globe, Zap, Radio, PhoneOff, MessageSquare, ArrowRight, DoorOpen, Home, Server, Hash, Plus, LayoutGrid, Play, Power, Layers, Menu, X } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  if (isInRoom && !myStream) {
    return (
      <div className="min-h-screen neo-bg flex flex-col items-center justify-center p-6 text-center">
         <div className="neo-card max-w-lg w-full p-8 md:p-12 flex flex-col gap-8 items-center relative animate-in zoom-in-95 duration-200">
           ...
           <div className="space-y-4">
               <h2 className="text-3xl md:text-4xl font-black text-black uppercase">Mic Check</h2>
           ...
           <div className="flex flex-col gap-3 w-full">
                <Button onClick={enableVoice} variant="glow" fullWidth className="h-14 md:h-16 text-xl">
                    ENABLE MICROPHONE
                </Button>
  ...
  if (isInRoom) {
      return (
        <div className="min-h-screen neo-bg text-black flex flex-col overflow-hidden relative">
          {/* Room Navbar */}
          <nav className="h-16 md:h-20 px-4 md:px-12 flex items-center justify-between border-b-[3px] border-black bg-white sticky top-0 z-40 shrink-0">
            <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
               <div className={`shrink-0 w-3 h-3 md:w-4 md:h-4 rounded-full border-2 border-black ${isSignalConnected ? 'bg-[#4ade80]' : 'bg-red-500 animate-pulse'}`}></div>
               <div className="flex flex-col overflow-hidden">
                   <span className="font-black text-lg md:text-xl tracking-tighter leading-none truncate">{activeServerData ? activeServerData.name : 'TEMPORARY ROOM'}</span>
                   {activeServerData && <span className="text-[10px] md:text-xs font-bold text-gray-500 truncate">HOST: {activeServerData.ownerName}</span>}
               </div>
               
               <div className="h-6 w-[2px] bg-gray-300 mx-1 hidden md:block"></div>
               <div className="bg-black text-white px-2 py-0.5 rounded text-[10px] md:text-xs font-bold font-mono hidden md:block">
            ...
            <div className="flex items-center gap-2">
               <button 
                  onClick={leaveRoom}
                  className="bg-white hover:bg-red-50 text-black border-2 border-black rounded-lg p-2 md:px-4 md:py-2 md:h-10 flex items-center gap-2 transition-all active:scale-95 shadow-[2px_2px_0px_0px_#000] active:shadow-none"
               >
                 <LogOut className="w-5 h-5 md:hidden" />
                 <span className="hidden md:inline font-bold text-sm">LEAVE ROOM</span>
               </button>
            </div>
          </nav>

          <main className="flex-1 overflow-y-auto p-4 md:p-8 max-w-[1400px] mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 pb-32">
            
            {/* LEFT COLUMN: Me & Invite */}
            <div className="lg:col-span-4 flex flex-col gap-4 md:gap-6">
               {/* My Card */}
               <div className="neo-card p-4 md:p-6 flex flex-col gap-4 md:gap-6 bg-white relative">
                  <div className="flex flex-col items-center gap-4">
                     <div className="relative">
                        <div className="w-24 h-24 md:w-32 md:h-32 border-[3px] border-black rounded-2xl overflow-hidden bg-white shadow-[4px_4px_0px_0px_#000]">
                            <img src={user.photoURL || ''} alt="Me" className="w-full h-full object-cover" />
                        </div>
                        {/* Status Badge */}
                        <div className="absolute -bottom-3 -right-3 bg-[#C4B5FD] border-2 border-black px-2 py-1 text-[10px] md:text-xs font-bold rounded-md shadow-[2px_2px_0px_0px_#000]">
                            YOU
                        </div>
                        {isMuted && (
                     ...
                     <div className="text-center w-full">
                        <h2 className="text-xl md:text-2xl font-black uppercase">{user.displayName}</h2>
                     </div>

                     <div className="w-full bg-[#f3f4f6] border-2 border-black rounded-lg p-3 md:p-4 mt-1">
               ...
               {/* Invite Section */}
               <div className="neo-card p-4 md:p-6 flex flex-col gap-3 md:gap-4 bg-[#FFFBEB]">
                  <div className="flex items-center gap-3 border-b-2 border-black pb-3">
                      <div className="p-1.5 bg-black text-white rounded">
                        <Share2 className="w-4 h-4 md:w-5 md:h-5" />
                      </div>
                      <h3 className="font-bold text-base md:text-lg uppercase">Invite Squad</h3>
                  </div>
                  
                  {myId ? (
                    <div className="flex gap-2">
                        <div className="flex-1 bg-white border-2 border-black rounded-lg p-2 font-mono text-xs md:text-sm truncate flex items-center px-3">
                            {inviteLink.replace(/^https?:\/\//, '')}
                        </div>
                         <Button variant="secondary" onClick={copyLink} className="!h-10 !w-10 !p-0 !rounded-lg !border-2 shrink-0">
            ...
            <div className="lg:col-span-8 flex flex-col gap-4 md:gap-6">
               <div className="flex items-center justify-between">
                  <h2 className="text-2xl md:text-3xl font-black flex items-center gap-3">
                     <span className="w-8 h-8 md:w-10 md:h-10 bg-[#A7F3D0] border-2 border-black flex items-center justify-center rounded-lg shadow-[3px_3px_0px_0px_#000]">
               ...
               {connections.length === 0 ? (
                 <div className="flex-1 min-h-[300px] md:min-h-[400px] neo-card border-dashed flex flex-col items-center justify-center text-center gap-6 p-8 md:p-12 bg-gray-50">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-white border-[3px] border-black flex items-center justify-center relative shadow-[6px_6px_0px_0px_#000]">
                       <Radio className="w-8 h-8 md:w-10 md:h-10 text-black/50" />
                    </div>
                    <div>
                      <h3 className="text-xl md:text-2xl font-black uppercase mb-1">Silence...</h3>
                      <p className="font-medium text-sm md:text-base text-gray-500 max-w-sm mx-auto">The room is empty. Share the invite link to start yapping.</p>
                    </div>
                 </div>
               ) : (
          ...
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-white border-[3px] border-black p-2 rounded-2xl shadow-[6px_6px_0px_0px_#000] z-50 flex items-center gap-3 md:gap-4 scale-90 md:scale-100">
             <button 
                onClick={toggleMic}
                className={`w-12 h-12 md:w-14 md:h-14 rounded-xl border-2 border-black flex items-center justify-center transition-all active:scale-95 shadow-[3px_3px_0px_0px_#000] active:shadow-none active:translate-y-[3px] active:translate-x-[3px] ${isMuted ? 'bg-red-200' : 'bg-white hover:bg-gray-50'}`}
                title={isMuted ? "Unmute" : "Mute"}
             >
                {isMuted ? <MicOff className="w-5 h-5 md:w-6 md:h-6 text-red-600" /> : <Mic className="w-5 h-5 md:w-6 md:h-6 text-black" />}
             </button>

             <button 
                onClick={leaveRoom}
                className="w-14 h-14 md:w-16 md:h-16 rounded-xl border-2 border-black flex items-center justify-center bg-red-500 hover:bg-red-600 transition-all active:scale-95 shadow-[3px_3px_0px_0px_#000] active:shadow-none active:translate-y-[3px] active:translate-x-[3px]"
                title="End Call"
             >
                <PhoneOff className="w-6 h-6 md:w-8 md:h-8 text-white fill-current" />
             </button>

             <button 
                onClick={() => setIsChatOpen(!isChatOpen)}
                className={`w-12 h-12 md:w-14 md:h-14 rounded-xl border-2 border-black flex items-center justify-center transition-all active:scale-95 shadow-[3px_3px_0px_0px_#000] active:shadow-none active:translate-y-[3px] active:translate-x-[3px] ${isChatOpen ? 'bg-[#FDE047]' : 'bg-white hover:bg-gray-50'}`}
                title="Chat"
             >
  ...
  // --- DASHBOARD (LOBBY) ---
  return (
    <div className="min-h-screen neo-bg flex flex-col md:flex-row text-black pb-20 md:pb-0">
        
        {/* MOBILE HEADER */}
        <header className="md:hidden h-16 bg-white border-b-[3px] border-black flex items-center justify-between px-4 sticky top-0 z-30 shrink-0">
             <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-[#FDE047] border-2 border-black rounded flex items-center justify-center shadow-[2px_2px_0px_0px_#000]">
                    <Zap className="w-5 h-5 text-black fill-current" />
                </div>
                <span className="font-black text-lg tracking-tight">VOICE_SYNC</span>
             </div>
             <div className="w-8 h-8 rounded border-2 border-black overflow-hidden bg-gray-200">
                  <img src={user.photoURL || ''} alt="User" className="w-full h-full object-cover" />
             </div>
        </header>

        {/* DESKTOP SIDEBAR */}
        <div className="hidden md:flex w-64 bg-white border-r-[3px] border-black flex-col sticky top-0 h-screen z-20">
            <div className="h-20 flex items-center px-6 border-b-[3px] border-black">
                <div className="w-10 h-10 bg-[#FDE047] border-2 border-black rounded-lg flex items-center justify-center mr-3 shadow-[2px_2px_0px_0px_#000]">
                    <Zap className="w-6 h-6 text-black fill-current" />
                </div>
                <span className="font-black text-xl tracking-tight">VOICE_SYNC</span>
            </div>
        ...
        <div className="flex-1 p-4 md:p-10 overflow-y-auto">
            ...
            {currentView === 'BROWSE' && (
                <div className="max-w-5xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="flex flex-col gap-2">
                        <h1 className="text-3xl md:text-5xl font-black uppercase">Browse Servers</h1>
                        <p className="text-lg md:text-xl font-medium text-gray-500">Join a persistent room or create your own.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                                ...
                                <div className="p-4 md:p-6 flex flex-col flex-1 gap-4">
                                    <div>
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="font-black text-lg md:text-xl uppercase leading-tight">{server.name}</h3>
                                            {server.isOnline ? (
                        ...
                        {servers.length === 0 && (
                            <div className="col-span-full py-12 text-center text-gray-500 font-bold text-lg border-[3px] border-dashed border-gray-300 rounded-xl bg-white/50">
            ...
            {currentView === 'MY_SERVERS' && (
                <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="flex flex-col gap-2">
                        <h1 className="text-3xl md:text-5xl font-black uppercase">My Servers</h1>
                        <p className="text-lg md:text-xl font-medium text-gray-500">Manage your owned servers.</p>
                    </div>

                    <div className="space-y-4">
                        {servers.filter(s => s.ownerId === user.uid).map(server => (
                             <div key={server.id} className="neo-card p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-6 bg-white">
                                <div className="flex items-center gap-4 w-full md:w-auto">
                                    <div className={`w-14 h-14 md:w-16 md:h-16 border-[3px] border-black rounded-lg flex items-center justify-center shadow-[2px_2px_0px_0px_#000] shrink-0 ${server.category === 'GAMING' ? 'bg-[#C4B5FD]' : 'bg-[#A7F3D0]'}`}>
                                        <Server className="w-6 h-6 md:w-8 md:h-8 text-black" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-black text-xl uppercase truncate">{server.name}</h3>
                                        <p className="font-medium text-gray-500 truncate text-sm">{server.description}</p>
                                        <div className="flex gap-2 mt-1">
                                            <span className="text-[10px] md:text-xs font-bold bg-gray-100 px-2 py-0.5 rounded border border-gray-300">{server.category}</span>
                                            {server.isOnline && <span className="text-[10px] md:text-xs font-bold bg-green-100 text-green-700 px-2 py-0.5 rounded border border-green-200">LIVE</span>}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3 w-full md:w-auto">
                                    <Button onClick={() => startServer(server)} variant="glow" className="flex-1 md:flex-none" fullWidth={true}>
            ...
            {currentView === 'CREATE' && (
                <div className="max-w-2xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                     <div className="flex flex-col gap-2">
                        <h1 className="text-3xl md:text-5xl font-black uppercase">Create Server</h1>
                        <p className="text-lg md:text-xl font-medium text-gray-500">Setup a permanent room.</p>
                    </div>

                    <div className="neo-card p-6 md:p-8 bg-white flex flex-col gap-6">
                        <div className="space-y-2">
        ...
        <nav className="md:hidden fixed bottom-0 w-full bg-white border-t-[3px] border-black z-40 flex justify-around items-center h-20 pb-2 shadow-[0px_-4px_10px_rgba(0,0,0,0.05)]">
             <button 
                onClick={() => setCurrentView('BROWSE')}
                className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-all ${currentView === 'BROWSE' ? 'text-black transform -translate-y-1' : 'text-gray-400'}`}
             >
                <div className={`p-2 rounded-lg border-2 ${currentView === 'BROWSE' ? 'bg-[#FDE047] border-black shadow-[2px_2px_0px_0px_#000]' : 'border-transparent'}`}>
             ...
             <button 
                onClick={() => setCurrentView('MY_SERVERS')}
                className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-all ${currentView === 'MY_SERVERS' ? 'text-black transform -translate-y-1' : 'text-gray-400'}`}
             >
                 <div className={`p-2 rounded-lg border-2 ${currentView === 'MY_SERVERS' ? 'bg-[#FDE047] border-black shadow-[2px_2px_0px_0px_#000]' : 'border-transparent'}`}>
                    <Layers className="w-6 h-6" />
                 </div>
                 <span className="text-[10px] font-bold uppercase">My Servers</span>
             </button>
             
             <button 
                onClick={() => setCurrentView('CREATE')}
                className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-all ${currentView === 'CREATE' ? 'text-black transform -translate-y-1' : 'text-gray-400'}`}
             >
                 <div className={`p-2 rounded-lg border-2 ${currentView === 'CREATE' ? 'bg-[#FDE047] border-black shadow-[2px_2px_0px_0px_#000]' : 'border-transparent'}`}>
...
const ParticipantCard: React.FC<{ connection: any, onDisconnect: (id: string) => void }> = ({ connection, onDisconnect }) => {
  ...
  return (
    <div className={`neo-card p-4 md:p-6 transition-all duration-300 relative group ${isSpeaking ? 'bg-[#F0FDFA]' : ''}`}>
       ...
       <div className="flex items-start justify-between mb-4">
          <div className="relative">
             <div className="w-12 h-12 md:w-16 md:h-16 rounded-lg bg-gray-200 border-2 border-black overflow-hidden">
       ...
       <div className="mb-4">
          <h4 className="font-black text-lg md:text-xl truncate uppercase" title={connection.peerId}>
            {connection.peerId.substring(0, 12)}
          </h4>
          <span className="text-[10px] md:text-xs font-bold bg-black text-white px-2 py-0.5 rounded">
             REMOTE USER
          </span>
       </div>
